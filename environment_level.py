...
def update_layout(game):
    # Get window dimensions (they remain fixed unless changed by our key presses)
...
def update_metrics(game):
    ...
    game['feasible_solution'] = (game['feasible_total_sum'] and game['feasible_min_release']
                                 and game['feasible_ramp_up'] and game['feasible_ramp_down'])
...
def draw_timer(game):
    ...
    feas_text = "Infeasible" if ((not game['feasible_solution']) or (revenue_pct > 100)) else "Feasible!"
    feas_color = (200, 0, 0) if ((not game['feasible_solution']) or (revenue_pct > 100)) else (0, 200, 0)
...
def draw_bar_graph(game):
    ...
    # Dashed lines at y=0.2 and y=2.0
    ...
    legend_x = game['bar_graph_area'].right - legend_width - int(game['window_width'] * 0.02)
    legend_y = game['bar_graph_area'].y + int(game['window_height'] * 0.02)
    ...
    dash_start = (legend_rect.x + int(legend_width * 0.07), legend_rect.y + int(legend_height * 0.3))
    dash_end = (legend_rect.x + int(legend_width * 0.2), legend_rect.y + int(legend_height * 0.3))
    ...
    screen.blit(price_text, (legend_rect.x + int(legend_width * 0.3), legend_rect.y + int(legend_height * 0.2)))
    pygame.draw.rect(screen, (31, 119, 180), (legend_rect.x + int(legend_width * 0.07), legend_rect.y + int(legend_height * 0.6), int(legend_width * 0.15), int(legend_height * 0.3)))
    rel_text = game['font'].render("Releases (AF)", True, get_text_color(game))
    screen.blit(rel_text, (legend_rect.x + int(legend_width * 0.3), legend_rect.y + int(legend_height * 0.7)))
...
def draw_message(game):
    if game['message']:
        ...
        if game.get('level_complete'):
            ...
            lc_btn_y = btn_y + lc_btn_height + 10
        ...
        if rect.collidepoint(pos):
            ...
            if label == "Restart":
                ...
                game['message'] = "\n".join(textwrap.wrap(
                    ...
                    "both a feasible solution and at least a 90% optimality. The optimality must be below 100%.", width=80))
